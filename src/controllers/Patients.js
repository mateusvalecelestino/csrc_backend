...
class Patients {
  async index(req, res) {
    try {
      const { count: totalPatients, rows: data } =
    ...
    try {
      ...
      if (!isInt(id))
        return res
          ...
          .json({ message: 'Id de paciente inválido.' });
      ...
      if (!patient)
        return res
          .status(httpStatusCode.BAD_REQUEST)
          .json({ message: 'Paciente não existe.' });
  ...
  async create(req, res) {
    ...
    try {
      ...
      if (!created)
        return res
          .status(httpStatusCode.BAD_REQUEST)
          .json({ message: 'Paciente já existe.' });
    ...
    try {
      ...
      if (similarPatients.length > 0)
        return res.status(httpStatusCode.BAD_REQUEST).json({
          message: 'Já existe paciente com dados similares.',
        });
