...
class Specialties {
  async index(req, res) {
    try {
      ...
      const { count: totalSpecialties, rows: data } =
        await Specialty.findAndCountAll({
          attributes: ['id', 'name'],
    ...
    try {
      ...
      if (!isInt(id))
        return res
          ...
          .json({ message: 'id de especialidade inválido.' });
      ...
      if (!specialty)
        return res
          .status(httpStatusCode.BAD_REQUEST)
          .json({ message: 'Especialidade não existe.' });
