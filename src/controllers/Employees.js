...
class Employees {
    async index(req, res) {
        try {
            ...
            const { count: totalEmployees, rows: data } =
    ...
    async show(req, res) {
        try {
            ...
            if (!employee)
                return res
                    .status(httpStatusCode.BAD_REQUEST)
                    .json({ message: 'Funcionário não existe.' });
    ...
    async create(req, res) {
        ...
        try {
            ...
            if (
                !(await Role.findByPk(employee.role_id, { attributes: ['id'] }))
            )
                return res
                    .status(httpStatusCode.BAD_REQUEST)
                    .json({ message: 'Cargo de funcionário não existe.' });
            if (
                !(await Specialty.findByPk(employee.specialty_id, {
                    attributes: ['id'],
                }))
            )
                return res.status(httpStatusCode.BAD_REQUEST).json({
                    message: 'Especialidade de funcionário não existe.',
                });

            // Salva o employee
            await employee.save({ transaction: t });

            const { id, full_name, birth_date, gender, order_number, user_id } =
                employee;
    ...
    async put(req, res) {
        ...
        try {
            ...
            if (
                ...
                !(await Role.findByPk(role_id, { attributes: ['id'] }))
            )
                return res
                    .status(httpStatusCode.BAD_REQUEST)
                    .json({ message: 'Cargo de funcionário inválido.' });
            if (
                !Number.isInteger(specialty_id) ||
                !(await Specialty.findByPk(specialty_id, {
                    attributes: ['id'],
                }))
            )
                return res.status(httpStatusCode.BAD_REQUEST).json({
                    message: 'Especialidade de funcionário inválida.',
                });
            ...
            const { id, full_name, birth_date, gender, order_number } =
                employee;
            await t.commit(); // Finalização da transaction
            return res.json({
                employee: {
    ...
    async patch(req, res) {
        ...
        try {
            ...
            if (!Number.isInteger(active))
                return res
                    .status(httpStatusCode.BAD_REQUEST)
                    .json({ message: 'Estado de utilizador inválido.' });
            ...
            if (!user)
                return res
                    ...
                    .json({ message: 'Funcionário não existe.' });
            await user.update(
                { active, updated_by: req.userId },
                { transaction: t }
            );
            await t.commit();
            const { id, full_name, updated_by } = user;
            return res.json({
                employee: { id, full_name, active, updated_by },
            });
        } catch (error) {
            await t.rollback();
            errorHandler(error, req, res);
